...
class GOT10KDataset(BaseDataset):
    ...
    def __init__(self, split):
        ...
        if split == 'test' or split == 'val':
            self.base_path = os.path.join(self.env_settings.got10k_path)
        else:
    ...
    # def _get_sequence_list(self, split):
    #     print('{}list.txt'.format(self.base_path))
    #     with open('{}list.txt'.format(self.base_path)) as f:
    #         sequence_list = f.read().splitlines()
    #         # TODO: select test seqs by numbers here, if you need
    #         sequence_list = []
    #         for num in [91]:
    #         # for num in [11]:
    #             # for num in [11, 13, 18, 70, 125, 139, 143, 150, 164]:
    #             # Format the number to 6 digits with leading zeros
    #             sequence_str = f'GOT-10k_Test_{num:06d}'
