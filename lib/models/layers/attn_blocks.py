...
from lib.models.layers.Lora import LoRA_Mlp
from lib.models.layers.attn import Attention, LoRA_Attention
...
class CEBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, keep_ratio_search=1.0, lora_r=4,
                 lora_alpha=1.0, use_lora=True):
        super().__init__()
        ...
        if use_lora:
            ...
            self.mlp = LoRA_Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                                lora_r=lora_r, lora_alpha=lora_alpha)
        else:
