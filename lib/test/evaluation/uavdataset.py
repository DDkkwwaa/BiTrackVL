...
class UAVDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_info):
        # ✅ 修正路径（去掉多余 data_seq/UAV123）
        sequence_path = sequence_info['path'].replace('data_seq/UAV123/', '').replace('data_seq/', '')
        anno_path = sequence_info['anno_path']
        anno_path_full = f"{self.base_path}/{anno_path}"
        ...
        frames = [
            f"{self.base_path}/{sequence_path}/{frame_num:0{nz}d}.{ext}"
            for frame_num in range(start_frame + init_omit, end_frame + 1)
        ]

        ground_truth_rect = load_text(str(anno_path_full), delimiter=',', dtype=np.float64, backend='numpy')
