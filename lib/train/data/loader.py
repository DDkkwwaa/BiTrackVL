...
def slt_collate(batch):
    """
        Input: TensorDict List of one batch, len = batch_size
               TensorDict[0].keys(): odict_keys(['template_images', 'template_anno', 'search_images', 'search_anno', 'dataset', 'test_class'])

        Output: collated batch data
    """
    ret = {}
    # keys = ['template_images', 'template_anno', 'search_images', 'search_anno', 'dataset', 'test_class']
    # Use keys from the first element to support dynamic keys like 'object_meta'
    keys = list(batch[0].keys())

    for k in keys:

        if k in ['template_anno', 'search_anno']:
            # stack annotations to [B, num, 4] tensors
            nested_tensors = [elem[k] for elem in batch]
            stacked_per_sample = [torch.stack(sample_tensors) for sample_tensors in nested_tensors]
            ret[k] = torch.stack(stacked_per_sample)

        elif k in ['template_images', 'search_images']:
            nested_lists = [elem[k] for elem in batch]
            ret[k] = list(zip(*nested_lists))

        elif k == 'object_meta':
            # Collate object_meta (list of dicts -> dict of lists)
            # batch[i]['object_meta'] is an OrderedDict
            meta_list = [elem[k] for elem in batch]
            if len(meta_list) > 0 and isinstance(meta_list[0], dict):
                collated_meta = {}
                # Collect all unique keys from all metas to handle missing keys
                all_keys = set()
                for m in meta_list:
                    all_keys.update(m.keys())

                for meta_key in all_keys:
                    # Use None for missing keys to avoid KeyError
                    collated_meta[meta_key] = [m.get(meta_key, None) for m in meta_list]
                ret[k] = collated_meta
            else:
                ret[k] = meta_list

        else:
            ret[k] = [elem[k] for elem in batch]
