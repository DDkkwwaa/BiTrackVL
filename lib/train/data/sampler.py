...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 num_search_frames, num_template_frames=1, num_previous_frames=0, processing=no_processing,
                 frame_sample_mode='causal',
                 processing_mode='preprocess_all', train_cls=False, pos_prob=0.5, max_jump=70):
        """
        args:
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                if self.frame_sample_mode == "consecutive":
                    # 寻找足够数量的连续可见帧
                    total_frames_needed = self.num_template_frames + self.num_search_frames

                    # 随机选择一个起始可见帧位置
                    start_idx = self._sample_visible_ids(visible, num_ids=1, min_id=0,
                                                         max_id=len(visible) - self.num_search_frames - 1)[0]

                    valid_frames = [start_idx]

                    # 向后寻找可见帧
                    current_idx = start_idx
                    while len(valid_frames) < total_frames_needed and current_idx < len(visible) - 1:
                        current_idx += 1
                        if visible[current_idx]:
                            valid_frames.append(current_idx)

                    # 如果向后找不到足够的帧，则尝试向前寻找
                    current_idx = start_idx
                    while len(valid_frames) < total_frames_needed and current_idx > 0:
                        current_idx -= 1
                        if visible[current_idx]:
                            valid_frames.insert(0, start_idx)
                            # 更新 start_idx 为当前找到的最前面的帧
                    ...
                    # 分配模板帧和搜索帧
                    template_frame_ids = valid_frames[:self.num_template_frames]
                    search_frame_ids = valid_frames[self.num_template_frames:total_frames_needed]
                    ...
                    while f1_idx is None or f2_idx is None or f3_idx is None or f4_idx is None:
                        ...
                        f1_idx = self._sample_visible_ids(visible, min_id=start_idx + 1,
                                                          max_id=min(start_idx + this_max_jump,
                                                                     len(visible) - self.num_search_frames + 1),
                                                          num_ids=1)[0]
                        if f1_idx is not None:
                            f2_idx = self._sample_visible_ids(visible, min_id=f1_idx + 1,
                                                              max_id=min(f1_idx + this_max_jump,
                                                                         len(visible) - self.num_search_frames + 2),
                                                              num_ids=1)[0]

                            if f2_idx is not None:
                                f3_idx = self._sample_visible_ids(visible, min_id=f2_idx + 1,
                                                                  max_id=min(f2_idx + this_max_jump,
                                                                             len(visible) - self.num_search_frames + 3),
                                                                  num_ids=1)[0]
                                if f3_idx is not None:
                                    f4_idx = self._sample_visible_ids(visible, min_id=f3_idx + 1,
                                                                      max_id=min(f3_idx + this_max_jump,
                                                                                 len(visible) - self.num_search_frames + 4),
                                                                      num_ids=1)[0]
                                    if f4_idx is not None:
                                        f5_idx = self._sample_visible_ids(visible, min_id=f4_idx + 1,
                                                                          max_id=min(f4_idx + this_max_jump,
                                                                                     len(visible) - self.num_search_frames + 5),
                                                                          num_ids=1)[0]
                            else:
                                maxJump = maxJump - 5 if maxJump > 5 else maxJump - 1
                        else:
                    ...
                    raise NotImplementedError("Not implemented frame sample mode!")
            else:
            ...
            try:
                ...
                if self.processing_mode == "frame_by_frame":
                    # there is no mask data in datasets which only provide bounding box
                    data = TensorDict({'template_images': template_frames,
                                       ...
                                       'object_meta': meta_obj_test})
                    valid = True
                elif self.processing_mode == "preprocess_all_frame":
                    ...
                    raise NotImplementedError("Not implemented frame processing mode!")
            except:
                # raise Exception
                valid = False
        ...
        while not valid:
            ...
            try:
                ...
                data = TensorDict({'template_images': template_frames,
                                   ...
                                   'dataset': dataset.get_name(),
                                   'test_class': meta_obj_test.get('object_class_name'),
                                   'caption': meta_obj_train.get('caption')})
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        while not enough_visible_frames:
            ...
            if is_video_dataset:
                ...
                # Eliminate invalid bbox (width < 1 or height < 1) in COCO
                first_bbox = seq_info_dict['bbox'][0]
                enough_visible_frames = True if first_bbox[2] > 1 and first_bbox[3] > 1 else False
