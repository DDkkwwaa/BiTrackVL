...
def bbox_iou_xywh(box1, box2):
    """
    计算两个框的 IOU，输入为 [x, y, w, h] 格式（左上角 + 宽高）
    支持 batch 输入：box1 和 box2 都应为 [N, 4] 的 tensor
    """
    # 转为 [x1, y1, x2, y2]
    box1_xyxy = torch.cat([box1[:, :2], box1[:, :2] + box1[:, 2:]], dim=1)
    box2_xyxy = torch.cat([box2[:, :2], box2[:, :2] + box2[:, 2:]], dim=1)
    ...
    iou = inter_area / (union + 1e-6)  # 避免除以0

    return iou
import torch

def bbox_iou_cxcywh(box1, box2):
    """
    输入框格式：[cx, cy, w, h]，支持 batch，输出 IOU
    box1: Tensor[N, 4]
    box2: Tensor[N, 4]
    """
    device = box2.device
