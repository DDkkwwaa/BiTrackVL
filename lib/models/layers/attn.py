...
from lib.models.layers.Lora import LoRALinear
...
class LoRA_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 rpe=False, z_size=7, x_size=14, lora_r=4, lora_alpha=1.0):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = head_dim ** -0.5
        ...
        # 替换为 LoRA 线性层
        self.qkv = LoRALinear(dim, dim * 3, lora_r=lora_r, lora_alpha=lora_alpha, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = LoRALinear(dim, dim, lora_r=lora_r, lora_alpha=lora_alpha)
        self.proj_drop = nn.Dropout(proj_drop)
